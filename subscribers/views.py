...
def my_leads(request):
    ...
    # Method 1: Direct assigned_to se (single assignment)
    # **FIX: Exclude replaced leads for this user**
    real_estate_leads = RealEstateLead.objects.filter(assigned_to=user).exclude(
        is_replaced=True, replaced_for_user=user
    )
    mba_leads = OnlineMBA.objects.filter(assigned_to=user).exclude(
        is_replaced=True, replaced_for_user=user
    )
    study_abroad_leads = StudyAbroad.objects.filter(assigned_to=user).exclude(
        is_replaced=True, replaced_for_user=user
    )
    forex_trade_leads = ForexTrade.objects.filter(assigned_to=user).exclude(
        is_replaced=True, replaced_for_user=user
    )
